...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'iotct', 'IotCT', and 'BOILERPLATE'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.iotct/g),
        replacement: `game.${this.propName}`,
      },
      {
        pattern: new RegExp(/flags\.iotct/g),
   ...
   * Rename files.
   *
   * Rename files that had iotct in their name, such as
   * css/iotct.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*iotct*.*`).then((files) => {
      files.forEach((oldPath) => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(
          oldPath,
          `${directory}/${file.replaceAll("iotct", this.packageName)}`,
          (err) => {
            if (err) throw err;
          }
        );
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-iotct-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    // Glob IotCT's files so that we can process them.
    glob("*", { ignore: ["node_modules/**"] }).then((files) => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace iotct name mentions in files.
      generator.replaceFileContents();
      // Rename files that had iotct in their name.
